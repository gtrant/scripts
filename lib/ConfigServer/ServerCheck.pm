...
sub phpcheck {
	...
	foreach my $key ("version","enable_dl","disable_functions","ini_set","register_globals") {
		...
		if ($key eq "version") {
			...
			if (-e "/usr/local/cpanel/version" and ref $cpconf eq 'HASH') {
				$status = 0;
				unless ($cpconf->{nobodyspam}) {$status = 1}
				&addline($status,"WHM: Allow 'Nobody' Email","You should prevent the user 'nobody' from sending mail.<br><small><i>WHM &gt; <a href='$cpurl/scripts2/tweaksettings' target='_blank'>Tweak Settings</a> &gt; Prevent \"nobody\" from sending mail</i></small>");
			}
		}
		if ($key eq "enable_dl") {
			my $status = 0;
